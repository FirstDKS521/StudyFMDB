...
NSAssert([self tableIsExist:modelClass autoCloseDB:NO], classNameTip);\
}
...
- (BOOL)updateModel:(id)model byId:(NSString *)aId {
    return [self modifyModel:model byId:aId autoCloseDB:YES];
}
        ...
        if (![self tableIsExist:[model class] autoCloseDB:NO]) {
            ...
            if (success) {
                ...
                if ([[judgeModle valueForKey:@"aId"] isEqualToString:fl_dbId]) {
                    BOOL updataSuccess = [self modifyModel:model byId:fl_dbId autoCloseDB:NO];
                    if (autoCloseDB) {
                        [self.dataBase close];
                    }
...
- (BOOL)modifyModel:(id)model byId:(NSString *)aId autoCloseDB:(BOOL)autoCloseDB {
    if ([self.dataBase open]) {
